...
(function(root, factory) {
  ...
  var config = {
    ...
    warnTimeLimit:        60,                                // 2 Minutes warning period
    ...
    warnTitle:            'Session Timeout Warning',                  // setting to null will remove the title bar
    warnMessage:          'For security reasons, your session is about to expire due to inactivity in:',
    warnStayAliveButton:  'Stay Connected',
    warnLogoutButton:     'Logout',                           // Set to null to disable
    warnCountdownMessage: '{timer}',          // Set to null to disable see doc on how to set
    ...
    redirectUrl:        'index.php?modfunc=logout&reason=session_expire',       //URL if no action is taken after the warning/lock screen timeout
    logoutUrl:          'index.php?modfunc=logout',          //URL if the user clicks "Logout" on the warning/lock screen
    logoutAutoUrl:      'index.php?modfunc=logout',       // URL for secondary tabs that received an automatic logout trigger (to help avoid race conditions)
  ...
  function time2txt(secs) {
    ...
    if (minLeft <= 15) {
      ...
      var timeTxt = minLeft > 0 ? minLeft + ' mins ' : ' ';
      timeTxt += secsLeft + ' secs';
      return timeTxt;
    }
    if (minLeft <= 75) {
